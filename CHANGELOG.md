...
- **The autohint gate was per font, not per glyph** ([#1](https://github.com/typedev/ufo-tdkit-tools/issues/1), second defect). `detect_font_source` returns a source as soon as **one** glyph has hints, and the pipeline only called the autohinter when the whole font came up empty. A partially hinted master — the normal shape of a FontLab source, where hints are authored on base forms and not on composites — was therefore treated as fully hinted, and every unhinted glyph went into the OTF unhinted (`source=public_ps glyphs=829 with_hints=408 autohinted=False`). `process_font` gained an `autohint` argument: `"fill"` (new default) autohints exactly the drawable glyphs the chosen source does not cover and keeps the authored hints of the rest; `"all"` discards authored hints and re-hints everything; `"off"` restores the old leave-it-alone behaviour and turns "no hints anywhere" into a failure instead of silently hinting the whole font. The autohinter runs with `glyphList` restricted to the gap glyphs and `explicitGlyphs` set, so its own `maxSegments` heuristic cannot skip complex glyphs.
...
- **Legacy `kern` table synthesis** (`compilation.legacy_kern`, also re-exported as `ufo_tdkit_tools.add_legacy_kern`). Microsoft Word and other Office rendering paths read kerning from the old-style `kern` table rather than from GPOS, so fonts built by ufo2ft/fontmake lose kerning there. `add_legacy_kern(ttf)` flattens the font's GPOS `kern` feature into a format-0 `kern` table and rewrites the binary so that only `kern` changes (GPOS is flattened on a throwaway lazy `TTFont`; every other table is written back verbatim and `head.modified` is preserved — `head.checkSumAdjustment` is the one unavoidable side effect). Pairs resolve first-wins in feature order, values are clamped to int16, and the table splits across several subtables past the uint16 `nPairs` limit. Contextual lookups inside the `kern` feature (kerning exceptions) have no format-0 equivalent and are skipped with a log line instead of being mistaken for pair positioning — reading them as `PairPos` used to raise `AttributeError: PairSet` and abort the whole flatten. Depends only on fontTools, so it works without the `compilation` extra.
- **`ProcessResult` build counters.** `autohinted_count` (glyphs the autohinter filled in), plus `otf_glyphs_hinted` / `otf_glyphs_total` taken from the compiler's `stats` — what actually reached the binary, which is what a build-level regression guard should assert on. The CLI prints them per file (`... -> otf 823/829`).
- **`--autohint {fill,all,off}` CLI flag** on `optimize-otf`, forwarded to `process_font`.
- **`ps_hints.batch.glyphs_missing_source(font, source)`** — names of drawable glyphs (contours or components) with no hints in a source; `export_all_from_processed` gained an optional `names` filter.
