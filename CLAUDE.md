...
4. **`pipeline.py`** — Single public entry point (`process_font`). Takes any binary or UFO input and produces an OTF + UFO pair. Resolves hint source per `hint_source` arg (auto-detect or explicit), optionally runs the optimizer, normalizes hints into default `autohint.v2`, then compiles. When the chosen source has zero hinted glyphs (binary input with unhinted CFF/TTF, or UFO + `auto` with no hints anywhere), runs `afdko.otfautohint.hintFiles` on the working UFO to generate hints; `ProcessResult.autohinted = True` records that this happened. Explicit `hint_source` (`v2` / `processed` / `public_ps`) with no hints still fails. Layer hygiene rule: a processedglyphs layer present on input survives to output; one created by the pipeline (as a working buffer or by the autohinter) is removed before save. Returns `ProcessResult` dataclass.
